...
class SQLiteManager:
    ...
    def optimize_database(self):
        ...
        try:
            ...
            print("🔧 データベース最適化設定を適用しました")
        except sqlite3.Error as e:
            print(f"⚠️ 最適化設定エラー: {e}")
    ...
    def vacuum_database(self) -> bool:
        ...
        try:
            ...
            end_time = time.time()
            print(f"✅ データベース最適化完了 ({end_time - start_time:.2f}秒)")
            return True
        except sqlite3.Error as e:
    ...
    def execute_query(self, query: str) -> Tuple[List, Optional[str]]:
        ...
        try:
            ...
            if query.strip().upper().startswith(('INSERT', 'UPDATE', 'DELETE', 'CREATE', 'DROP', 'ALTER')):
                ...
                return [], f"✅ クエリ実行完了 ({affected}行影響, {end_time - start_time:.3f}秒)"
            else:
                results = self.cursor.fetchall()
                return results, f"✅ {len(results)}行取得 ({end_time - start_time:.3f}秒)"
...
class EnhancedSQLiteCLI:
    ...
    def display_menu(self):
        """メインメニューを表示"""
        print("\n" + "="*50)
        print("🗃️  Enhanced SQLite Database Manager")
        print("="*50)
        print("1.  📂 データベースを開く")
        ...
        print("5.  📊 データを表示")
        print("6.  💻 SQLクエリを実行")
        print("7.  📈 テーブルパフォーマンス分析")
        print("8.  🗂️  インデックスを作成")
        print("9.  🔧 データベース最適化(VACUUM)")
        print("10. 💾 データベースバックアップ")
    ...
    def run(self):
        """メインループ"""
        print("🚀 Enhanced SQLite Database Manager を開始します")
        ...
        while True:
            ...
            if choice == '1':
                ...
                print("❌ 無効な選択です。1-13の数字を入力してください。")
    ...
    def analyze_performance(self):
        ...
        if not analysis.get('has_indexes') and analysis.get('row_count', 0) > 1000:
            print("\n💡 推奨: レコード数が多いため、インデックスの作成を検討してください")
    ...
    def vacuum_database(self):
        """データベース最適化"""
        confirm = input("⚠️ データベースを最適化しますか？ (y/N): ").strip()
        if confirm.lower() == 'y':
